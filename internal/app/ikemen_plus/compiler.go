	...
	return c.stateParam(is, prefix+"trans", func(data string) error {
		...
		if !afterImage {
			if err := c.stateParam(is, prefix+"alpha", func(data string) error {
				...
				if len(bes) > 1 {
					exp[1] = bes[1]
					if tt != TT_alpha && tt != TT_add && tt != TT_add1 {
						exp[1].append(OC_pop)
					}
				}
				switch tt {
				case TT_alpha, TT_add, TT_add1:
					if len(bes) <= 1 {
...
func (c *Compiler) stateCompile(states map[int32]StateBytecode,
	...
	for ; c.i < len(c.lines); c.i++ {
		...
		} else if strings.Contains(filename, "data/") && c.stateNo < 0 {
			for _, ctrl := range states[c.stateNo].block.ctrls {
				sbc.block.ctrls = append(sbc.block.ctrls, ctrl)
			}
			states[c.stateNo] = *sbc
		}
	}
...
func (c *Compiler) stateCompileZ(states map[int32]StateBytecode,
	...
	for {
		...
		case "statedef":
			...
			if _, ok := states[c.stateNo]; !ok {
				states[c.stateNo] = *sbc
			} else if strings.Contains(filename, "common") && c.stateNo < 0 {
				for _, ctrl := range states[c.stateNo].block.ctrls {
					sbc.block.ctrls = append(sbc.block.ctrls, ctrl)
				}
				sbc.numVars += states[c.stateNo].numVars
				states[c.stateNo] = *sbc
			}
		case "function":
			name := c.scan(&line)
