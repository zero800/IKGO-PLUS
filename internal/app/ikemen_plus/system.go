...
func (s *System) fight() (reload bool) {
	...
	reset := func() {
		...
		s.nextRound()
		if s.round == 1 {
			s.bgm.Open(s.stage.bgmusic[0])
		} else if s.stage.bgmusic[1] != "" && (s.roundType[0] >= RT_Deciding || s.roundType[1] >= RT_Deciding) {
			s.bgm.Open(s.stage.bgmusic[1])
		} else if bgmstate == 2 {
			s.bgm.Open(s.stage.bgmusic[0])
		}
		bgmbool[0], bgmbool[1], bgmstate = false, false, 0
		if s.stage.bgmusic[2] != "" {
...
func newSelect() *Select {
	return &Select{columns: 5, rows: 2, randomscl: [...]float32{1, 1},
		...
		vsportrait: [...]int16{9000, 1}, vportrait: [...]int16{9000, 2}, bgmvolume: -1}
}
...
func (s *Select) GetStageBGM(n int) ([3]string, int32) {
	n %= len(s.stagelist) + 1
	if n < 0 {
		n += len(s.stagelist) + 1
	}
	return s.stagelist[n-1].bgmusic, s.stagelist[n-1].bgmvolume
}
func (s *Select) addCahr(def string) {
	...
	for i < len(lines) {
		...
		case "arcade":
			if arcade {
				arcade = false
				sc.intro_storyboard = is["intro.storyboard"]
				sc.ending_storyboard = is["ending.storyboard"]
			}
		}
	}
	sc.sprite = sprite
...
func (s *Select) AddStage(def string) error {
	...
	for i < len(lines) {
		...
		case "music":
			if music {
				music = false
				ss.bgmusic[0] = is["bgmusic"]
				ss.bgmusic[1] = is["bgmusicalt"]
				ss.bgmusic[2] = is["bgmusiclife"]
				ss.bgmvolume = 100
				is.ReadI32("bgmvolume", &ss.bgmvolume)
			}
		}
	}
	return nil
...
func (s *Select) AddSurvivalChar() bool {
	m, n, pl := 0, 0, 0
	for i := 0; i < int(sys.numSimul[1]); i++ {
		for s.charlist[n].def == "randomselect" || len(s.charlist[n].def) == 0 {
			m++
			if m > 100000 {
				return false
