...
type backGround struct {
	...
	scalestart   [2]float32 //not implemented yet
	scaledelta   [2]float32 //not implemented yet
	zoomdelta    float32    //not implemented yet
}
...
func newBackGround(sff *Sff) *backGround {
	return &backGround{anim: *newAnimation(sff), delta: [...]float32{1, 1},
		...
		scalestart: [...]float32{1, 1}, scaledelta: [...]float32{0, 0}, zoomdelta: 1}
}
func readBackGround(is IniSection, link *backGround,
	sff *Sff, at AnimationTable, camstartx, camstarty float32) *backGround {
	bg := newBackGround(sff)
	bg.camstartx, bg.camstarty = camstartx, camstarty
	typ, t := is["type"], 0
	if len(typ) == 0 {
		return bg
	}
	...
	is.ReadF32("zoomdelta", &bg.zoomdelta)
	return bg
}
...
func (bg backGround) draw(pos [2]float32, scl, bgscl, lclscl float32,
	...
	x := bg.start[0] + bg.xofs - (pos[0]/stgscl[0]+bg.camstartx)*bg.delta[0] +
		bg.bga.offset[0]
	y := bg.start[1] - (pos[1]/stgscl[1]+bg.camstarty)*bg.delta[1] + bg.bga.offset[1]
	...
	rect[1] = int32(math.Floor(float64(((float32(rect[1])-
		(pos[1]+bg.camstarty)*bg.windowdelta[1])*wscl[1] - shakeY + float32(sys.gameHeight-240)) *
		sys.heightScale)))
...
func newStage(def string) *Stage {
	s := &Stage{def: def, leftbound: float32(math.NaN()),
		...
		bgmvolume: 100, bgmlife: 30, bgmlast: 0}
...
func loadStage(def string) (*Stage, error) {
	...
	if sec := defmap["camera"]; len(sec) > 0 {
		...
		sec[0].ReadI32("starty", &sys.cam.starty)
		...
		sec[0].ReadI32("boundlow", &sys.cam.boundlow)
		sec[0].ReadF32("verticalfollow", &sys.cam.verticalfollow)
		sec[0].ReadI32("tension", &sys.cam.tension)
		sec[0].ReadI32("tensionhigh", &sys.cam.tensionhigh)
		sec[0].ReadI32("tensionlow", &sys.cam.tensionlow)
		sec[0].ReadI32("floortension", &sys.cam.floortension)
		sec[0].ReadI32("overdrawhigh", &sys.cam.overdrawhigh)
		sec[0].ReadI32("overdrawlow", &sys.cam.overdrawlow)
		sec[0].ReadI32("cuthigh", &sys.cam.cuthigh)
		sec[0].ReadI32("cutlow", &sys.cam.cutlow)
		sec[0].ReadF32("startzoom", &sys.cam.ztopscale)
		sec[0].ReadF32("zoomout", &sys.cam.ZoomMin)
		sec[0].ReadF32("zoomin", &sys.cam.ZoomMax)
	}
	...
	if sec := defmap["music"]; len(sec) > 0 {
		if sys.sel.bgmusic[0] == "" {
			s.bgmusic[0] = sec[0]["bgmusic"]
		} else {
			s.bgmusic[0] = sys.sel.bgmusic[0]
		}
		if sys.sel.bgmusic[1] == "" {
			s.bgmusic[1] = sec[0]["bgmusicalt"]
		} else {
			s.bgmusic[1] = sys.sel.bgmusic[1]
		}
		if sys.sel.bgmusic[2] == "" {
			s.bgmusic[2] = sec[0]["bgmusiclife"]
		} else {
			s.bgmusic[2] = sys.sel.bgmusic[2]
		}
		if sys.sel.bgmvolume == -1 {
			sec[0].ReadI32("bgmvolume", &s.bgmvolume)
		} else {
			s.bgmvolume = sys.sel.bgmvolume
		}
	...
	for _, bgsec := range defmap["bg"] {
		...
		s.bg = append(s.bg, readBackGround(bgsec, bglink,
			s.sff, s.at, float32(sys.cam.startx), float32(sys.cam.starty)))
	}
	bgcdef := *newBgCtrl()
	i = 0
	for i < len(lines) {
...
func (s *Stage) draw(top bool, x, y, scl float32) {
	...
	scl2, boundlow := s.localscl*scl, float32(Max(sys.cam.boundlow, sys.cam.boundhigh))
	if pos[1] > boundlow {
