...
//func newBgm() *Bgm {
//return &Bgm{openReq: make(chan string, 1)}
//}
...
func (bgm *Bgm) Open(file string) {
	bgm.clear()
	if bgm.openFile(file) {
		// Init the Speaker with the SampleRate of the format and a buffer size of 1/10s
		speaker.Init(bgm.format.SampleRate, bgm.format.SampleRate.N(time.Second/10))

		// Channel, which will signal the end of the playback.
		//playing := make(chan struct{})

		// Now we Play our Streamer on the Speaker
		speaker.Play(beep.Seq(bgm.dec, beep.Callback(func() {
...
func (bgm *Bgm) IsWav() bool {
	return filepath.Ext(bgm.filename) == ".wav"
}
...
func (bgm *Bgm) openFile(file string) bool {
	bgm.clear()
	var err error
	if bgm.fp, err = os.Open(file); err != nil {
		return false
	}
	bgm.filename = file
	return bgm.restart()
...
func (bgm *Bgm) restart() bool {
	if bgm.fp == nil {
		return false
	}
	_, err := bgm.fp.Seek(0, 0)
	chk(err)
	// Decode the File
	if bgm.IsMp3() {
		if bgm.dec, bgm.format, err = mp3.Decode(bgm.fp); err != nil {
			bgm.clear()
			return false
		}
	} else if bgm.IsWav() {
		if bgm.dec, bgm.format, err = wav.Decode(bgm.fp); err != nil {
			bgm.clear()
			return false
		}
	} else if bgm.IsFlac() {
		if bgm.dec, bgm.format, err = flac.Decode(bgm.fp); err != nil {
			bgm.clear()
			return false
		}
	} else {
...
func (s *Snd) play(gn [2]int32) bool {
	...
	c.SetVolume(256 * sys.wavVolume / 100)
	c.sound = s.Get(gn)
	return c.sound != nil
}
