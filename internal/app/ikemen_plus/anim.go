...
// AnimFrameLine uses a single line from an animation inside a screenpack or .air file
// in order to get its data or manipulate it.
//
// For example:
// 0,0, 0,0, 10
// AnimFrameLine can validate if that line is ok and then it can extract information.
type AnimFrameLine struct {
	Line string
}

func newAnimFrameLine() *AnimFrameLine {
	return &AnimFrameLine{
		Line: "",
	}
}

// IsValid verify if a single line of an animation is ok
func (afl *AnimFrameLine) IsValid() bool {
	if len(afl.Line) == 0 || (afl.Line[0] < '0' || '9' < afl.Line[0]) && afl.Line[0] != '-' {
		return false
	}

	ary := strings.SplitN(afl.Line, ",", -1)
	if len(ary) < 5 || len(ary) > 10 {
		return false
	}
...
// GetSprGroup returns the sprite group of an frame animation
func (afl *AnimFrameLine) GetSprGroup() int16 {
	ary := strings.SplitN(afl.Line, ",", -1)

	return int16(Atoi(ary[0]))
}

// GetSprNum returns the sprite number of an frame animation
func (afl *AnimFrameLine) GetSprNum() int16 {
	ary := strings.SplitN(afl.Line, ",", -1)

	return int16(Atoi(ary[1]))
}

//GetPosX returns the sprite X position of an frame animation
func (afl *AnimFrameLine) GetPosX() int16 {
	ary := strings.SplitN(afl.Line, ",", -1)

	return int16(Atoi(ary[2]))
}

//GetPosY returns the sprite X position of an frame animation
func (afl *AnimFrameLine) GetPosY() int16 {
	ary := strings.SplitN(afl.Line, ",", -1)

	return int16(Atoi(ary[3]))
}

//GetTime returns the frame duration
func (afl *AnimFrameLine) GetTime() int32 {
	ary := strings.SplitN(afl.Line, ",", -1)

	return Atoi(ary[4])
}

//GetFlipH returns the horizontal flip.
//It doesn't apply the position on X
func (afl *AnimFrameLine) GetFlipH() int8 {
	ary := strings.SplitN(afl.Line, ",", -1)
...
//GetFlipV returns the vertical flip.
//It doesn't apply the position on Y
func (afl *AnimFrameLine) GetFlipV() int8 {
	ary := strings.SplitN(afl.Line, ",", -1)
...
//GetTransparency returns the alpha source and destination.
func (afl *AnimFrameLine) GetTransparency() (byte, byte) {

	ary := strings.SplitN(afl.Line, ",", 7)
	...
	var srcAlpha, dstAlpha byte

	switch {
	case a == "a1":
		srcAlpha, dstAlpha = 255, 128
	case len(a) > 0 && a[0] == 's':
		srcAlpha, dstAlpha = 1, 255
	case len(a) >= 2 && a[:2] == "as":
		if len(a) > 2 && a[2] >= '0' && a[2] <= '9' {
			...
			if i < len(a) && a[i] == 'd' {
				...
				if i < len(a) && a[i] >= '0' && a[i] <= '9' {
					...
					if srcAlpha == 1 && dstAlpha == 255 {
						srcAlpha = 0
					} else if srcAlpha == 255 && dstAlpha == 1 {
						dstAlpha = 0
					}
				}
			}
	...
	case len(a) > 0 && a[0] == 'a':
		srcAlpha, dstAlpha = 255, 1

	default:
		srcAlpha, dstAlpha = 255, 0
	}
...
//GetExtras returns an array with
//Xscale, Yscale, Angle
func (afl *AnimFrameLine) GetExtras() [][]float32 {
	...
	if len(ary) > 1 {
		extras = make([][]float32, 3)
		f, err := strconv.ParseFloat(ary[1], 32)
		if err != nil {
			f = 1
		}
		extras[2] = append(extras[2], float32(f)) // X-Scale
		if len(ary) > 2 {
			f, err := strconv.ParseFloat(ary[2], 32)
			if err != nil {
				f = 1
			}
			extras[2] = append(extras[2], float32(f)) // Y-Scale
			if len(ary) > 3 {
				f, err := strconv.ParseFloat(ary[3], 32)
				if err != nil {
					f = 0
				}
				extras[2] = append(extras[2], float32(f*math.Pi/180)) // Angle
			}
		}
	}
...
// AnimFrame stores the information of a single frame animation
...
//ReadAnimFrame creates an AnimFrame from an animation line
func ReadAnimFrame(line string) *AnimFrame {

	afl := AnimFrameLine{Line: line}
	...
	af := newAnimFrame()
	af.Group = afl.GetSprGroup()
	af.Number = afl.GetSprNum()
	af.H = afl.GetFlipH()
	af.V = afl.GetFlipV()
	af.X = afl.GetPosX() * int16(af.H)
	af.Y = afl.GetPosY() * int16(af.V)
	af.Time = afl.GetTime()
	af.SrcAlpha, af.DstAlpha = afl.GetTransparency()
	af.Ex = afl.GetExtras()
