...
func scriptCommonInit(l *lua.LState) {
	...
	luaRegister(l, "setWavVolume", func(l *lua.LState) int {
		sys.wavVolume = int32(numArg(l, 1))
		return 0
	})
}
...
func systemScriptInit(l *lua.LState) {
	...
	luaRegister(l, "textImgSetColor", func(*lua.LState) int {
		...
		alphaSrc, alphaDst := int32(-1), int32(-1)
		if l.GetTop() >= 5 {
			alphaSrc = int32(numArg(l, 5))
		}
		if l.GetTop() >= 6 {
			alphaDst = int32(numArg(l, 6))
		}
		ts.fnt.SetColor(int32(numArg(l, 2)), int32(numArg(l, 3)), int32(numArg(l, 4)), alphaSrc, alphaDst)
		return 0
	})
	...
	luaRegister(l, "loadDebugFont", func(l *lua.LState) int {
		f, err := loadFnt(strArg(l, 1), int32(0))
		if err != nil {
			l.RaiseError(err.Error())
		}
		sys.debugFont = f
	...
	luaRegister(l, "game", func(l *lua.LState) int {
		...
		survival := false
		if sys.gameMode == "survival" || sys.gameMode == "survivalcoop" || sys.gameMode == "netplaysurvivalcoop" {
			survival = true
		}
		oldWins, oldDraws := sys.wins, sys.draws
		load := func() error {
			if !survival {
				sys.loader.runTread()
			} else {
				...
				//sys.sel.ClearSelected()
				sys.loadMutex.Lock()
				sys.sel.selected = [2][][2]int{}
				sys.loadMutex.Unlock()
				sys.sel.selectedStageNo = -1
				sys.sel.bgmusic = [...]string{"", "", ""}
				sys.sel.bgmvolume = -1
				//sys.loadStart()
				//sys.loaderReset()
				//				/*sys.round, sys.wins, sys.roundsExisted, sys.roundType, sys.challenger = 1, [2]int32{}, [2]int32{}, [2]RoundType{}, 0*/
				oldWins, oldDraws = sys.wins, sys.draws
				sys.wins[0] = 0
				//sys.loader.reset()
				...
				go sys.loader.load()
				/*	defer func() { sys.loader.loadExit <- sys.loader.state }()
					...
					for !stageDone || !allCharDone() {
						for i, b := range charDone {
							if !b {
								result := sys.loader.loadChar(i)
								if result > 0 {
									charDone[i] = true
								} else if result < 0 {
									sys.loader.state = LS_Error
									return
								}
							}
						}
				...
				*/
				...
				//selectChar
				sys.sel.AddSelectedChar(0, 0, 1)
				//sys.sel.AddSelectedChar(0, 1, 1)
				sys.sel.AddSelectedChar(1, 0, 1)
				//setStage
				sys.sel.SetStageNo(1)
				//selectStage
				sys.sel.SelectStage(1)

			}
			//here stage is loaded
		...
		for {
			...
			fight := func() (int32, error) {
				...
				sys.charList.clear()
				if survival {
					sys.wins, sys.draws = oldWins, oldDraws
					//if sys.tmode[1] == TM_Single {
					//}
					//if sys.tmode[1] == TM_Simul {
					//}
					//for i := 1; i < len(sys.chars); i += 2 {

					//}

					//fmt.Println(sys.sel.selected)
					//[[[1 1]] [[1 3]]] //p2 single
					//[[[1 1]] [[0 3] [1 3]]] //p2 simul2

					//for i := 0; i < len(sys.chars); i += 1 {
					//	if len(sys.chars[i]) > 0 {
					//		fmt.Println(sys.chars[i][0])
					//	}
					//}
					//sys.sel.AddSelectedChar(1, 2, 0)
					//fmt.Println(sys.sel.charlist)
					//sys.sel.AddSurvivalChar()
				}
				for i := 0; i < len(sys.chars); i += 2 { //p1: 0, 2, 4, 6
					if len(sys.chars[i]) > 0 {
						sys.chars[i][0].id = sys.newCharId()
					}
				}
				for i := 1; i < len(sys.chars); i += 2 { //p2: 1, 3, 5, 7
					if len(sys.chars[i]) > 0 {
						sys.chars[i][0].id = sys.newCharId()
					}
				}
				...
				if sys.round == 1 {
					if survival {
						sys.matchWins[0] = 9999
					} else if sys.tmode[1] == TM_Turns {
	...
	luaRegister(l, "getStageBGM", func(*lua.LState) int {
		bgmusic, bgmvolume := sys.sel.GetStageBGM(int(numArg(l, 1)))
		tbl := l.NewTable()
		for k, v := range bgmusic {
			tbl.RawSetInt(k+1, lua.LString(v))
		}
		l.Push(tbl)
		l.Push(lua.LNumber(bgmvolume))
		return 2
	})
	luaRegister(l, "setStageBGM", func(l *lua.LState) int {
		if l.GetTop() >= 2 {
			sys.sel.bgmvolume = int32(numArg(l, 2))
		}
		k := 0
		if l.GetTop() >= 3 {
			k = int(numArg(l, 3))
		}
		sys.sel.bgmusic[k] = strArg(l, 1)
		return 0
	})
